...
def test_read_dataset_file_accepts_documented_string_locations(tmp_path):
    """DatasetHandle.location string reads should match Path-based dispatch."""
    frame = pd.DataFrame({
        "underlying_symbol": ["TSLA", "NVDA"],
        "is_stale_quote": pd.Series([False, True], dtype="boolean"),
        "unused_column": [1, 2],
    ...
    columns = ["is_stale_quote", "missing_column"]
    csv_result = read_dataset_file(str(csv_path), columns=columns)
    parquet_result = read_dataset_file(str(parquet_path), columns=columns)
