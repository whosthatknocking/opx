...
| Field | yfinance | massive | marketdata |
| --- | --- | --- | --- |
| `next_earnings_date` | Transformed: best-effort minimum upcoming date from Yahoo `info` earnings timestamps and `calendar` earnings values | Blank: expected because event fetching is not implemented for this provider | Transformed: selected from `stocks/earnings/{symbol}/`; `date` is preferred as a confirmed event date when present, otherwise the minimum upcoming `reportDate` estimate is used. Rows whose `reportedEPS` is already populated, or whose confirmed `date` is already in the past, are excluded |
| `next_earnings_date_is_estimated` | Transformed/Blank: `info.isEarningsDateEstimate` when Yahoo exposes it for the chosen future earnings date, otherwise blank | Blank: expected because event fetching is not implemented for this provider | Derived: `False` when the selected date came from Market Data `date`; `True` when it came from estimate fallback `reportDate`; blank when no future earnings date is available |
| `next_earnings_date_source` | Derived: `yfinance` when Yahoo supplies the selected future earnings date | Blank: expected because event fetching is not implemented for this provider | Derived: `marketdata.date` for confirmed dates, or `marketdata.reportDate` for estimate fallback dates |
| `next_earnings_date_confidence` | Derived: `estimated`, `confirmed`, or `unknown` from Yahoo's estimate flag when a date is present | Blank: expected because event fetching is not implemented for this provider | Derived: `confirmed` for `marketdata.date`, or `estimated` for `marketdata.reportDate` fallback |
